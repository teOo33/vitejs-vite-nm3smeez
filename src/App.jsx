...
import {
  ...
  Search,
  Activity,    // اضافه شده
  TrendingUp,  // اضافه شده
  AlertCircle, // اضافه شده
  CheckCircle2 // اضافه شده
} from 'lucide-react';
...
import {
  ...
  Cell,
  Legend,
  AreaChart, // اضافه شده برای زیبایی بیشتر
  Area       // اضافه شده
} from 'recharts';
...
// =================================================================================
// 🎨 Tailwind از CDN (همراه با انیمیشن‌های جدید)
// =================================================================================
const useTailwind = () => {
  useEffect(() => {
    ...
    if (!document.getElementById('tailwind-cdn')) {
      ...
      // اضافه کردن استایل‌های سفارشی برای انیمیشن پس‌زمینه
...
const UserAvatar = ({ name, size = 'md' }) => {
    ...
    return (
        <div className={`${sizeClasses} rounded-full bg-gradient-to-br ${colors[colorIndex]} text-white flex items-center justify-center font-bold shadow-md ring-2 ring-white`}>
            {safeName.charAt(0)}
        </div>
    );
};
...
export default function App() {
  ...
  // -------------------- Analytics & Smart Churn Logic (NEW) --------------------
  ...
  // 🌟 Logic for Churn Prediction
  const churnRisks = useMemo(() => {
      // Helper to parse Shamsi date roughly or assume created_at is usable if standard. 
      // Since your code saves Shamsi strings like "1402/10/20", we need to be careful.
      // For this example, we will look at the last 20 issues to avoid heavy parsing, 
      // OR we just count occurrence if you don't have a reliable date parser.
      // A better way without a library: Assume 'id' order implies time.
      // Let's filter user who has > 3 issues in the top 50 recent records (approx 1 week for active apps)
      
      const recentIssues = issues.slice(0, 100); // Check last 100 issues
      const userCounts = {};
  ...
  const handleSave = async (e) => {
    ...
    if (modalType === 'issue') {
      ...
      payload = {
        ...
        type: formData.type,
        status: formData.status || 'باز', // پیش‌فرض: باز
      ...
      payload = {
        ...
        status: formData.status || 'فریز', // پیش‌فرض: فریز
        subscription_status: formData.subscription_status,
      ...
      payload = {
        ...
        status: formData.status || 'بررسی نشده', // پیش‌فرض: بررسی نشده
      ...
      payload = {
        ...
        category: formData.category,
        action: formData.action || 'در حال بررسی', // پیش‌فرض: در حال بررسی
  ...
  return (
    <div className="fixed inset-0 w-full h-full bg-[#F3F4F6] text-right font-sans flex overflow-hidden relative" dir="rtl">
      
      {/* 🔮 Background Blobs (زیباسازی) */}
      ...
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 right-0 z-40 h-full bg-white/80 border-l border-white/50 flex flex-col transition-transform duration-300 shadow-2xl backdrop-blur-xl md:relative ${isSidebarOpen ? 'translate-x-0 w-64' : 'translate-x-full md:translate-x-0 md:w-20'}`}>
        <div className="p-5 flex items-center justify-between border-b border-gray-100/50">
           <div className={`${isSidebarOpen ? 'block' : 'hidden md:hidden'} flex flex-col`}><span className="font-extrabold text-transparent bg-clip-text bg-gradient-to-l from-blue-600 to-purple-600 text-xl leading-none">وردست</span><span className="text-[10px] text-slate-400 mt-1">داشبورد هوشمند</span></div>
           <div className={`hidden md:flex flex-col ${!isSidebarOpen && 'md:hidden'}`}><span className="font-extrabold text-blue-700 text-lg leading-none">V</span></div>
          <button onClick={() => setSidebarOpen(!isSidebarOpen)} className="p-2 hover:bg-slate-50 rounded-xl border border-slate-100 mr-auto transition">{isSidebarOpen ? <X size={20} className="md:hidden"/> : <Menu size={20} />}<Menu size={20} className="hidden md:block"/></button>
        </div>
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
            {[{ id: 'dashboard', label: 'داشبورد', icon: LayoutDashboard }, { id: 'issues', label: 'مشکلات فنی', icon: AlertTriangle }, { id: 'frozen', label: 'اکانت فریز', icon: Snowflake }, { id: 'features', label: 'درخواست فیچر', icon: Lightbulb }, { id: 'refunds', label: 'بازگشت وجه', icon: CreditCard }, { id: 'profile', label: 'پروفایل کاربر', icon: User }].map((i) => (
                <button key={i.id} onClick={() => { setActiveTab(i.id); if(window.innerWidth < 768) setSidebarOpen(false); }} className={`w-full flex items-center gap-3 px-3 py-3 rounded-xl text-sm transition-all whitespace-nowrap overflow-hidden ${activeTab === i.id ? 'bg-gradient-to-l from-blue-50 to-white text-blue-700 font-bold border border-blue-100 shadow-sm' : 'text-slate-600 hover:bg-white/50'}`}>
                    <i.icon size={18} className="shrink-0" /><span className={`${!isSidebarOpen && 'md:hidden'} transition-opacity duration-200`}>{i.label}</span>
                </button>
            ))}
        </nav>
        <div className="p-4 text-xs text-center text-gray-400 border-t border-gray-100">{isConnected ? <span className="text-emerald-600 flex justify-center gap-1 font-bold items-center"><span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_10px_rgba(16,185,129,0.5)]" /><span className={`${!isSidebarOpen && 'md:hidden'}`}>آنلاین</span></span> : 'Off'}</div>
      </aside>

      <main className="flex-1 w-full h-full overflow-y-auto overflow-x-hidden px-4 sm:px-8 lg:px-10 py-6 relative z-10">
        <header className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3"><button onClick={() => setSidebarOpen(true)} className="md:hidden p-2 bg-white border border-gray-200 rounded-xl shadow-sm text-gray-600"><Menu size={20} /></button><div className="flex flex-col gap-1"><h1 className="text-xl sm:text-2xl font-extrabold text-slate-800">داشبورد پشتیبانی</h1></div></div>
          <div className="hidden sm:flex items-center gap-2 text-[11px] text-slate-500"><span className="px-3 py-1.5 rounded-full bg-white/60 backdrop-blur border border-white shadow-sm font-medium">امروز {new Date().toLocaleDateString('fa-IR', { weekday: 'long', year: 'numeric', month: '2-digit', day: '2-digit' })}</span></div>
        </header>

        {activeTab === 'dashboard' && (
          <section className="space-y-6">
             {/* Cards Row (Modern Glass Style) */}
             <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
              {[
                  ...
                  <div key={idx} className="bg-white/70 backdrop-blur-xl p-5 rounded-3xl shadow-sm border border-white flex flex-col justify-between h-32 relative overflow-hidden group hover:shadow-md transition duration-300">
            ...
            {/* 🌟 Churn & Charts Row (NEW) */}
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                
                {/* 🚨 Churn Risk Widget (New Feature) */}
                <div className="xl:col-span-1 bg-white/70 backdrop-blur-xl p-5 rounded-3xl shadow-sm border border-red-100 flex flex-col h-[350px]">
                ...
                {/* Charts */}
                <div className="xl:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4 h-[350px]">
                     <div className="bg-white/70 backdrop-blur-xl p-5 rounded-3xl shadow-sm border border-white flex flex-col">
